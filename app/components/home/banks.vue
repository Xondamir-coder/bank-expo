<template>
  <section id="home-banks" class="banks">
    <div class="banks__top">
      <h2 class="banks__title">{{ $t('home.banks.title') }}</h2>
      <p class="banks__text">
        {{ $t('home.banks.text') }}
      </p>
    </div>
    <div class="banks__list">
      <div v-for="(bank, index) in banks" :key="index" class="banks__item">
...
.banks {
  ...
  &__list {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(max(24rem, 150px), 1fr));
    gap: max(3.2rem, 20px);
